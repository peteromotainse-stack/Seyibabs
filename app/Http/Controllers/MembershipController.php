...
class MembershipController extends Controller
{
    ...
    public function convertPoints(Request $request)
    {
        $validated = $request->validate([
            'type' => 'required|in:subscription,visibility',
        ]);
        ...
        if ($validated['type'] === 'subscription') {
            if ($user->points < 5000) {
                return back()->withErrors(['points' => 'Insufficient points. Need 5,000 points.']);
            }
        ...
        if ($validated['type'] === 'visibility') {
            if ($user->points < 1000) {
                return back()->withErrors(['points' => 'Insufficient points. Need 1,000 points.']);
            }
            $user->decrement('points', 1000);
            $newScore = min(100, $user->visibility_score + 20);
            $user->update(['visibility_score' => $newScore]);
