...
class ReciprocityController extends Controller
{
    ...
    public function complete(Request $request, Campaign $campaign)
    {
        ...
        $alreadyDone = CampaignCompletion::where('campaign_id', $campaign->id)
            ->where('user_id', $user->id)
            ->exists();

        if ($alreadyDone) {
            return back()->with('error', 'Already completed this task.');
        }
        ...
        $newVisibility = min(100, $user->visibility_score + 2);
        $newEngagement = min(100, $user->engagement_rate + 0.5);
        $user->update(['visibility_score' => $newVisibility, 'engagement_rate' => $newEngagement]);
    ...
    public function ignore(Request $request, Campaign $campaign)
    {
        ...
        $user->update([
            'points'          => max(0, $user->points - 50),
            'visibility_score'=> max(0, $user->visibility_score - 10),
            'engagement_rate' => max(0, $user->engagement_rate - 2),
        ]);
