...
<div class="space-y-6 fade-in">
    <div class="grid md:grid-cols-3 gap-4">
        ...
        <div class="glass rounded-3xl p-6"><p class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Total Earned</p><p class="text-3xl font-black text-emerald-400">${{ number_format($totalEarned, 2) }}</p></div>
        <div class="glass rounded-3xl p-6"><p class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Total Withdrawn</p><p class="text-3xl font-black text-slate-300">${{ number_format($user->total_withdrawn, 2) }}</p></div>
    </div>

    <div class="grid lg:grid-cols-2 gap-6">
        <div class="glass rounded-3xl p-8">
            <h3 class="text-base font-bold text-white mb-6 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                Deposit Funds
            </h3>
            <form method="POST" action="{{ route('wallet.deposit') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="cursor-pointer"><input type="radio" name="gateway" value="paystack" class="sr-only peer" checked>
                            <div class="p-3 rounded-xl border-2 border-slate-700 peer-checked:border-emerald-500 peer-checked:bg-emerald-500/5 text-center transition-all"><p class="text-sm font-bold text-white">Paystack</p><p class="text-[10px] text-slate-500">Cards & Bank</p></div>
                        </label>
                        <label class="cursor-pointer"><input type="radio" name="gateway" value="flutterwave" class="sr-only peer">
                            <div class="p-3 rounded-xl border-2 border-slate-700 peer-checked:border-emerald-500 peer-checked:bg-emerald-500/5 text-center transition-all"><p class="text-sm font-bold text-white">Flutterwave</p><p class="text-[10px] text-slate-500">Mobile & Cards</p></div>
                        </label>
                    </div>
                </div>
        ...
        <div class="glass rounded-3xl p-8">
            <h3 class="text-base font-bold text-white mb-6 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7"/></svg>
                Withdraw Funds
            </h3>
        ...
        @else
            <div class="space-y-3">
                ...
                @php
                    ...
                    $c = $colors[$tx->type] ?? 'slate'; $s = $signs[$tx->type] ?? '';
                @endphp
                <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-900/40 hover:bg-slate-900/60 transition-colors">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-{{ $c }}-500/10 flex items-center justify-center"><span class="text-{{ $c }}-400 font-black">{{ $s }}</span></div>
                        <div><p class="text-sm font-semibold text-slate-200">{{ $tx->description }}</p><p class="text-xs text-slate-500">{{ \Carbon\Carbon::parse($tx->created_at)->format('M d, Y · H:i') }}</p></div>
                    </div>
