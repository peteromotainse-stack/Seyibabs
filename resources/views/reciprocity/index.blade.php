...
<div class="space-y-6 fade-in">
    ...
    @else
        <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            @php $creatorName=$task->user->name??'Unknown'; $creatorAvatar=$task->user->avatar??'https://ui-avatars.com/api/?name='.urlencode($creatorName).'&background=6366f1&color=fff&size=80'; @endphp
            <div class="glass rounded-[2rem] p-6 glass-hover ring-1 ring-indigo-500/20">
                ...
                <p class="text-sm text-slate-400 mb-5 line-clamp-2">{{ $task->description }}</p>
                <div class="space-y-2 mb-6">
                    @php $progress=$task->total_slots>0?(1-$task->remaining_slots/$task->total_slots)*100:0; @endphp
                    <div class="flex justify-between text-[10px] font-bold uppercase text-slate-500"><span>Community Progress</span><span>{{ $task->total_slots-$task->remaining_slots }}/{{ $task->total_slots }}</span></div>
                    <div class="h-1.5 w-full bg-slate-800/50 rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full" style="width:{{ $progress }}%"></div></div>
                </div>
                <div class="flex gap-3">
                    <a href="{{ $task->link }}" target="_blank" class="flex-1 flex items-center justify-center gap-2 bg-slate-800/50 hover:bg-slate-800 text-slate-300 py-3.5 rounded-2xl text-xs font-bold border border-slate-700/50 transition-all">Open</a>
                    ...
                    <form method="POST" action="{{ route('reciprocity.ignore',$task) }}">
                        @csrf
                        <button type="submit" class="px-3 py-3.5 rounded-2xl bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-400 text-xs font-bold transition-all" onclick="return confirm('Ignoring applies -50pts penalty. Continue?')">&times;</button>
                    </form>
                </div>
                <div class="mt-4 pt-3 border-t border-slate-800/30 text-center"><span class="text-[8px] font-black text-slate-500 uppercase tracking-widest">Mutual Reciprocity Active</span></div>
            </div>
            @endforeach
        </div>
    @endif
