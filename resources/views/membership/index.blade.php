...
<div class="space-y-8 fade-in">
            ...
            <div>
                ...
                @if($user->membership_status === 'ACTIVE')
                    <div class="flex items-center gap-3">
                        ...
                        <div><h3 class="text-2xl font-black text-white">Elite Member</h3><p class="text-indigo-400 text-sm font-semibold">All features unlocked</p></div>
                    </div>
                @else
                    <div class="flex items-center gap-3">
                        ...
                        <div><h3 class="text-2xl font-black text-white">Free Member</h3><p class="text-slate-400 text-sm">Upgrade to unlock all features</p></div>
                    </div>
                @endif
            </div>
            <div class="flex items-center gap-4">
                <div class="text-center"><p class="text-2xl font-black text-amber-400">{{ number_format($user->points) }}</p><p class="text-xs text-slate-500">Points</p></div>
                <div class="w-px h-12 bg-slate-800"></div>
                <div class="text-center"><p class="text-2xl font-black text-white">${{ number_format($user->wallet_balance, 2) }}</p><p class="text-xs text-slate-500">Balance</p></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-6">
        <div class="glass rounded-3xl p-8 border-2 border-indigo-500/30 relative overflow-hidden">
            ...
            <form method="POST" action="{{ route('membership.upgrade') }}">@csrf
        ...
        <div class="space-y-4">
            <div class="glass rounded-3xl p-6">
                ...
                <div class="flex items-center justify-between mb-4"><span class="text-sm text-slate-300">Required Points</span><span class="text-lg font-black {{ $user->points >= 5000 ? 'text-emerald-400' : 'text-rose-400' }}">5,000 pts</span></div>
                <div class="h-2 bg-slate-800 rounded-full mb-4"><div class="h-full bg-gradient-to-r from-amber-500 to-amber-400 rounded-full" style="width: {{ min(100, ($user->points / 5000) * 100) }}%"></div></div>
                <p class="text-xs text-slate-500 mb-4">You have {{ number_format($user->points) }} / 5,000 points</p>
                <form method="POST" action="{{ route('membership.convert-points') }}">@csrf
                    <input type="hidden" name="type" value="subscription">
                    <button type="submit" {{ $user->points < 5000 ? 'disabled' : '' }} class="w-full py-3.5 {{ $user->points >= 5000 ? 'bg-amber-600 hover:bg-amber-500' : 'bg-slate-800 cursor-not-allowed opacity-50' }} text-white font-bold rounded-2xl text-sm transition-all">
            ...
            <div class="glass rounded-3xl p-6">
                ...
                <div class="flex items-center justify-between mb-4">
                    <div><p class="text-xs text-slate-500">Current Score</p><p class="text-2xl font-black text-indigo-400">{{ $user->visibility_score }}/100</p></div>
                    <div class="text-right"><p class="text-xs text-slate-500">After boost</p><p class="text-2xl font-black text-white">{{ min(100, $user->visibility_score + 20) }}/100</p></div>
                </div>
                <form method="POST" action="{{ route('membership.convert-points') }}">@csrf
                    <input type="hidden" name="type" value="visibility">
                    <button type="submit" {{ $user->points < 1000 ? 'disabled' : '' }} class="w-full py-3.5 {{ $user->points >= 1000 ? 'bg-fuchsia-600 hover:bg-fuchsia-500' : 'bg-slate-800 cursor-not-allowed opacity-50' }} text-white font-bold rounded-2xl text-sm transition-all">
    ...
    <div class="glass rounded-3xl p-8 text-center">
        ...
        <div class="grid grid-cols-3 gap-4 max-w-md mx-auto mb-8">
            ...
            <div class="bg-{{ $color }}-500/10 border border-{{ $color }}-500/20 rounded-2xl p-3"><p class="text-xs font-bold text-{{ $color }}-400">{{ $feature }}</p></div>
            @endforeach
        </div>
        <div class="max-w-sm mx-auto">
            <div class="glass rounded-2xl p-5">
                ...
                <form method="POST" action="{{ route('membership.convert-points') }}">@csrf
                    <input type="hidden" name="type" value="visibility">
                    <button type="submit" {{ $user->points < 1000 ? 'disabled' : '' }} class="w-full py-3 {{ $user->points >= 1000 ? 'bg-fuchsia-600 hover:bg-fuchsia-500' : 'bg-slate-800 opacity-50 cursor-not-allowed' }} text-white font-bold rounded-xl text-sm transition-all">
