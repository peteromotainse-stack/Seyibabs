    ...
    <form method="POST" action="{{ route('campaigns.store') }}" class="space-y-6">
        ...
        <div class="glass rounded-3xl p-8 space-y-5">
            ...
            <div>
                ...
                <input type="url" name="link" value="{{ old('link') }}" placeholder="https://instagram.com/p/your-post" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors placeholder-slate-600">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Description</label>
                <textarea name="description" rows="3" placeholder="Describe what you want people to do..." class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors placeholder-slate-600 resize-none">{{ old('description') }}</textarea>
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Target Location</label>
                <input type="text" name="target_location" value="{{ old('target_location','Global') }}" placeholder="e.g. USA, Nigeria, Global" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors placeholder-slate-600">
            </div>
        </div>
        <div class="glass rounded-3xl p-8 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="reward_value" value="{{ old('reward_value','50') }}" min="0" step="0.01" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors">
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Total Slots</label>
                    <input type="number" name="total_slots" value="{{ old('total_slots','100') }}" min="1" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wide">Total Budget (USD)</label>
                    <input type="number" name="budget" value="{{ old('budget','0') }}" min="0" step="0.01" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-200 text-sm focus:outline-none focus:border-indigo-500 transition-colors">
                </div>
            </div>
            <label class="flex items-center gap-3 p-4 rounded-2xl bg-slate-900/50 border border-slate-800 cursor-pointer">
                ...
                <div><p class="text-sm font-bold text-white">Enable AI Advertisement</p><p class="text-xs text-slate-400">AI optimizes delivery for maximum reach and engagement</p></div>
        ...
        <div class="flex items-center justify-between p-5 glass rounded-2xl">
            <div><p class="text-xs text-slate-500">Your wallet balance</p><p class="text-lg font-black text-white">${{ number_format($user->wallet_balance, 2) }}</p></div>
            <button type="submit" class="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-4 rounded-2xl font-black text-sm shadow-lg shadow-indigo-600/20 transition-all">Launch Campaign</button>
        </div>
    </form>
</div>
@endsection
