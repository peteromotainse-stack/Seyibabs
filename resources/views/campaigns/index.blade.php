...
<div class="space-y-6 fade-in">
    ...
    @if($campaigns->isEmpty())
        <div class="glass rounded-3xl p-16 text-center">
            ...
            <a href="{{ route('campaigns.create') }}" class="inline-flex items-center gap-2 mt-6 bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-xl text-sm font-bold transition-all">
                Create Campaign
            </a>
        </div>
    @else
            ...
            <div class="glass rounded-[2rem] p-6 glass-hover relative overflow-hidden {{ $isMatch ? 'ring-1 ring-indigo-500/30' : '' }} {{ $isCash ? 'ring-1 ring-emerald-500/20' : '' }}">
                ...
                <div class="mt-4 pt-3 border-t border-slate-800/30 flex items-center justify-center gap-2">
                    <span class="text-[8px] font-black text-slate-500 uppercase tracking-widest">
                        {{ $isCash ? 'Instant Wallet Payout' : 'Mutual Reciprocity Active' }}
                    </span>
                </div>
            </div>
            @endforeach
