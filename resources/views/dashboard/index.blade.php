    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
            <div class="glass rounded-3xl p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between"><span class="text-sm text-slate-400">Total Earned</span><span class="text-sm font-bold text-emerald-400">${{ number_format($totalEarned, 2) }}</span></div>
                    <div class="flex items-center justify-between"><span class="text-sm text-slate-400">Total Withdrawn</span><span class="text-sm font-bold text-slate-300">${{ number_format($user->total_withdrawn, 2) }}</span></div>
                    <div class="flex items-center justify-between"><span class="text-sm text-slate-400">Active Campaigns</span><span class="text-sm font-bold text-indigo-400">{{ $activeCampaigns }}</span></div>
                    <div class="flex items-center justify-between"><span class="text-sm text-slate-400">Referral Code</span><span class="text-sm font-mono font-bold text-fuchsia-400">{{ $user->referral_code ?? 'N/A' }}</span></div>
                    <div class="flex items-center justify-between"><span class="text-sm text-slate-400">Warnings</span><span class="text-sm font-bold {{ $user->warnings > 0 ? 'text-rose-400' : 'text-emerald-400' }}">{{ $user->warnings }}</span></div>
                </div>
            </div>
            <div class="glass rounded-3xl p-6">
                ...
                <div class="flex justify-between text-xs text-slate-500">
                    <span>0</span><span class="font-bold text-indigo-400">{{ $user->visibility_score }}%</span><span>100</span>
                </div>
